...
// validSubjectTokenEnv is the environment a credential file generated by
// `pivb wif credentials --alias ro` produces.
func validSubjectTokenEnv() map[string]string {
	return map[string]string{
		envTokenType:         wif.TokenType,
		envAudience:          testAudience,
		envImpersonatedEmail: testTargetRO,
		envOutputFile:        "",
	}
}

func envWithout(mutate func(map[string]string)) map[string]string {
	env := validSubjectTokenEnv()
	mutate(env)
	return env
}

// runSubjectToken invokes the command with a fully specified protocol
// environment. Every variable is set explicitly — including the ones a case
// wants absent — so ambient state cannot leak in; subjecttoken.go reads them
// with os.Getenv, which cannot tell empty from unset.
...
func TestSubjectTokenSuccess(t *testing.T) {
	...
	stdout, stderr, err := runSubjectToken(t, writeConfig(t), socket, []string{"--alias", "ro"}, validSubjectTokenEnv())
	if err != nil {
	...
	if got[0] != wantReq {
		t.Errorf("daemon received %+v, want %+v", got[0], wantReq)
	}
}
...
func TestSubjectTokenEnvRejections(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			env:        envWithout(func(env map[string]string) { env[envTokenType] = "" }),
			wantStderr: envTokenType,
		},
		{
			name:       "token type jwt",
			args:       []string{"--alias", "ro"},
			env:        envWithout(func(env map[string]string) { env[envTokenType] = "urn:ietf:params:oauth:token-type:jwt" }),
			wantStderr: "only issues",
		},
		{
			name: "audience mismatch",
			args: []string{"--alias", "ro"},
			env: envWithout(func(env map[string]string) {
		...
		},
		{
			...
			args:       []string{"--alias", "ro"},
			env:        envWithout(func(env map[string]string) { env[envOutputFile] = "/tmp/pivb-cached-token.json" }),
...
func TestSubjectTokenConfigErrors(t *testing.T) {
	...
	t.Run("no config", func(t *testing.T) {
		...
		stdout, _, err := runSubjectToken(t, missing, socket, []string{"--alias", "ro"}, validSubjectTokenEnv())
		assertProtocolError(t, stdout, err, wifapi.CodeConfig)
	})

	t.Run("unknown alias", func(t *testing.T) {
		stdout, _, err := runSubjectToken(t, writeConfig(t), socket, []string{"--alias", "nosuch"}, validSubjectTokenEnv())
...
func TestSubjectTokenMalformedDaemonToken(t *testing.T) {
	...
	stdout, _, err := runSubjectToken(t, writeConfig(t), socket, []string{"--alias", "ro"}, validSubjectTokenEnv())
